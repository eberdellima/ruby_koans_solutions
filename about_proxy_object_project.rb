...
class Proxy
  ...
  def number_of_times_called(method_name)
    @messages << :number_of_times_called
    @messages.select { |message| message == method_name }.size
  end

  def upcase!
    @messages << :upcase!
    @object.upcase! if @object.respond_to?(:upcase!)
  end

  def split
    @messages << :split
    @object.split if @object.respond_to?(:split)
  end
  # WRITE CODE HERE
end
